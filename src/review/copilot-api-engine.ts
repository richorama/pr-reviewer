...
export class CopilotAPIEngine {
  ...
  private model = "gpt-4o"; // Default Copilot model
  ...
  constructor() {
    ...
    if (!token) {
      throw new Error(
        "No authentication token found. Set GH_TOKEN, GITHUB_TOKEN, or COPILOT_TOKEN environment variable."
      );
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          throw new Error(
            `Authentication failed. Ensure your token has 'copilot' scope. Status: ${response.status}, Error: ${errorText}`
          );
        }
